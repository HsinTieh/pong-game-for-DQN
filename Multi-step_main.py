...
if __name__ == "__main__":
    ...
    # 檢查是否有已訓練過的pkl檔案
    Q = load_model('training model pkl/Multi-step/Q_parame.pkl', Q)
    targetQ = load_model('training model pkl/Multi-step/target_Q_parame.pkl', targetQ)
    if USE_CUDA:
        Q.cuda()
        targetQ.cuda()
    ...
    for t in count():
        ...
        #實作multi-step   (s,a,r,s_ ,d,n_step, gamma):
        buffer.multi_step(obs_index, action, reward, obs_, done, n_step, paramter.gamma)
        ...
        if t > paramter.learning_start and \
            ...
            if t % print_log_freq == 0 and t > paramter.learning_ferg:
                ...
                sys.stdout.flush()
                # save model
                save_model('training model pkl/Multi-step/Q_params.pkl', Q)
                save_model('training model pkl/Multi-step/target_Q_parame.pkl',targetQ)               
