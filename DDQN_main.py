...
if __name__ == "__main__":
    ...
    env = wrappers.Monitor(env, 'results/DDQN', force = True, video_callable = lambda count: count%50 ==0)
    ...
    # 檢查是否有已訓練過的pkl檔案
    Q = load_model('training model pkl/DDQN/Q_parame.pkl', Q)
    targetQ = load_model('training model pkl/DDQN/target_Q_parame.pkl', targetQ)
    if USE_CUDA:
        Q.cuda()
        targetQ.cuda()
    ...
    for t in count():
        ...
        if t > paramter.learning_start and \
            ...
            '''
            :::DDQN:::
            Q(s,a) <-------> r + Q'(s', argmaxQ(s',a))

            '''
            #從抽出的batch observation中得出現在的Q值
            q_value = Q(obs_batch)
            next_q_value = Q(next_obs_batch)

            current_Q_values = q_value.gather(1,action_batch.unsqueeze(1)).squeeze(1)     
            # Q'(s', argmaxQ(s',a))
            next_max_q = targetQ(next_obs_batch).gather(1, next_q_value.max(1)[1].unsqueeze(1)).squeeze(1)
            next_Q_value = not_done_batch * next_max_q
            #TD value
            ...
            if t % print_log_freq == 0 and t > paramter.learning_ferg:
                ...
                sys.stdout.flush()
                # save model
                save_model('training model pkl/DDQN/Q_params.pkl', Q)
                save_model('training model pkl/DDQN/target_Q_parame.pkl',targetQ)               
