...
#引入prioritized的buffer
from agent.prioritized_replaybuffer import ReplayBuffer
...
if __name__ == "__main__":
    ...
    env = wrappers.Monitor(env, 'results/Prioritized', force = True, video_callable = lambda count: count%50 ==0)

    #step 2 : set DQN paramter
    eps = epsilon_decay()
    schedule = LinearSchedule()  
    #Priorities Schedule -------> 讓模型在前期比較偏向高利用率，後期偏向無偏   
    beta_start = 0.4
    beta_frames  = 100000
    '''
    1        -------------------
    ...
    # 檢查是否有已訓練過的pkl檔案
    Q = load_model('training model pkl/PrioritizedReplay/Q_parame.pkl', Q)
    targetQ = load_model('training model pkl/PrioritizedReplay/target_Q_parame.pkl', targetQ)
    if USE_CUDA:
        Q.cuda()
        targetQ.cuda()
    ...
    for t in count():
        ...
        if t > paramter.learning_start and \
            ...
            #取樣囉 ~~~~~~~~ ^____^
            obs_batch, action_batch, reward_batch, next_obs_batch, done_batch , weight_batch, index= buffer.pull_for_sample(paramter.batch_size, beta_by_frame(t))
            ...
            weight_batch = Variable(torch.from_numpy(weight_batch))
            ...
            # 1/m ∑ (y-y')^2 * w
            loss = F.smooth_l1_loss(current_Q_values, target_Q_value) * weight_batch
            # 將TD誤差反映至prios
            prios = loss + 1e-5
            loss = loss.mean()
            ...
            #updata prioiritized table
            if USE_CUDA:
                prios = prios.cuda()
            buffer.update_priorities(index, prios.data.numpy())
            ...
            if t % print_log_freq == 0 and t > paramter.learning_ferg:
                ...
                sys.stdout.flush()
                # save model
                save_model('training model pkl/PrioritizedReplay/Q_params.pkl', Q)
                save_model('training model pkl/PrioritizedReplay/target_Q_parame.pkl',targetQ)               
